...
function findBrowserExecutablePath(): string {
  ...
  throw new ApiError(
    'No se encontro un navegador compatible. Configura BROWSER_EXECUTABLE_PATH.',
    500,
  )
}
...
async function scrapeAttempt(
  ...
  try {
    ...
    if (readerUrl !== resolvedUrl) {
      ...
      if (readerStatus >= 400) {
        throw new ApiError(
          readerResponse?.statusText() || 'TMO rechazo la solicitud del lector.',
          readerStatus,
        )
      }
    }
    ...
    const pages = await page.$$eval('div.img-container img', (images) =>
      images
...
export async function getTmoChapterPagesWithBrowser(
  chapterUrl: string,
  referer?: string,
): Promise<MangaChapterPages> {

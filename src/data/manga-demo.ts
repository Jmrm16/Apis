...
function createPoster(title: string, tagline: string, palette: [string, string, string]): string {
...
function createPage(title: string, chapterLabel: string, pageNumber: number, palette: [string, string, string]): string {
  const [primary, secondary, accent] = palette
  const svg = `
...
function chapterId(seedId: string, index: number): string {
  return `${seedId}${String(index + 1).padStart(2, '0')}`
}

const seeds: DemoMangaSeed[] = [
  {
    id: '41021',
    slug: 'oshi-no-ko',
    libraryType: 'manga',
    ...
    description:
      'Ai Hoshino parecia inalcanzable, pero detras del brillo habia contratos, presion y un mundo dispuesto a devorar talentos. Esta ficha de demo sirve para dejar lista la seccion de manga con una estructura estable para home, detalle y lectura.',
  ...
  },
  {
    ...
    description:
      'Blue Lock mezcla competencia cerrada, paneles agresivos y capitulos pensados para cerrar siempre arriba. Aqui queda listo como una serie de lectura rapida y scroll vertical.',
    status: 'En publicacion',
    demography: 'Shounen',
    rating: '4.7',
  ...
  },
  {
    ...
    description:
      'Esta entrada conserva un tono mas contemplativo para equilibrar la home. El detalle prioriza atmosfera, capitulos y una lectura vertical sobria.',
    status: 'En publicacion',
    demography: 'Shounen',
    rating: '4.9',
  ...
  },
  {
    ...
    description:
      'Sakamoto Days entra aqui como la opcion mas ligera y cinetica del catalogo. La interfaz de lectura le queda bien porque cada pagina se entiende a simple scroll.',
    status: 'En publicacion',
    demography: 'Shounen',
    rating: '4.6',
  ...
  },
  {
    ...
    description:
      'Esta ficha sirve perfecto para la lectura integrada porque sus paginas combinan bloques grandes, diagonales y un contraste fuerte que luce bien en el tema oscuro.',
    status: 'En publicacion',
    demography: 'Shounen',
    rating: '4.8',
  ...
  },
  {
    ...
    description:
      'Deje un manhwa dentro del bloque para que tambien tengas variedad de tipo y una lectura que se sienta natural en scroll largo desde el primer dia.',
    status: 'Finalizado',
    demography: 'Accion',
    rating: '4.9',
...
const summaryById = new Map<string, MangaSummary>()
const detailByKey = new Map<string, MangaDetail>()
const readByKey = new Map<string, MangaReadData>()
const latestChapters: MangaRecentChapter[] = []

function createSummary(seed: DemoMangaSeed): MangaSummary {
  return {
    id: seed.id,
    slug: seed.slug,
...
function createChapter(seed: DemoMangaSeed, index: number, cover: string): MangaChapterSummary {
...
for (const seed of seeds) {
  ...
  const related = seed.relatedIds
    ...
    .filter((entry): entry is MangaSummary => Boolean(entry))
  ...
  chapters.forEach((chapter, index) => {
    const pages = Array.from({ length: 5 }, (_, pageIndex) =>
      createPage(seed.title, chapter.numberLabel, pageIndex + 1, seed.palette),
    )
...
export const mangaDemoHome: MangaHomeData = {

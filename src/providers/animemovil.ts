...
const FILTER_STATUS_MAP: Record<AnimeStatusCode, string> = {
  1: '2',
  2: '1',
  3: '3',
}
...
const FILTER_GENRE_MAP: Record<string, string> = {
  accion: '1',
  aventura: '23',
  comedia: '5',
  drama: '6',
...
function extractPath(baseUrl: string, value: string): string {
  try {
...
function extractSeriesSlug(baseUrl: string, value: string): string {
  return extractPath(baseUrl, value).replace(/^ver\//, '')
}

function extractEpisodeSlug(baseUrl: string, value: string): string {
  return extractPath(baseUrl, value).replace(/^ver\//, '')
}
...
function extractAnimeSlugFromEpisodeSlug(episodeSlug: string, episodeNumber: number): string {
...
function buildSeriesUrl(baseUrl: string, slug: string): string {
  return `${trimTrailingSlash(baseUrl)}/${slug.replace(/^\/+|\/+$/g, '')}`
}

function buildEpisodeUrl(baseUrl: string, animeSlug: string, episodeNumber: number): string {
  return `${trimTrailingSlash(baseUrl)}/ver/${animeSlug.replace(/^\/+|\/+$/g, '')}-${episodeNumber}`
}

async function requestAnimeMovilText(
  baseUrl: string,
  pathOrUrl: string,
  signal?: AbortSignal,
): Promise<string> {
...
function getFirstStatusValue(values: AnimeStatusCode[]): string | null {
  for (const value of values) {
...
function normalizePaginationValue(value: number): number {
  return Number.isFinite(value) && value > 0 ? Math.floor(value) : 1
}

function buildDirectoryUrl(baseUrl: string, params: SearchParams): string {
...
function extractListItemBlocks(sectionHtml: string): string[] {
...
function parseDirectoryCards(html: string, baseUrl: string): DirectoryCard[] {
  const listHtml =
    html.match(/<ul class="grid-animes directorio">([\s\S]*?)<\/ul>/i)?.[1] ??
...
function parseDirectoryPage(
  ...
  const currentPage =
    Number(html.match(/class="active page-item active"><a class="page-link"[^>]*>(\d+)<\/a>/i)?.[1] ?? '') ||
    requestedPage
...
function parseLatestEpisodes(html: string, baseUrl: string): EpisodeSummary[] {
  const latestSection =
    html.match(
      /<h2><i class="icon fas fa-burn icon-pink"><\/i>Últimos Episodios<\/h2>[\s\S]*?<ul class="grid-animes">([\s\S]*?)<\/ul>/i,
    )?.[1] ?? ''
...
function parseCover(html: string, baseUrl: string): string | undefined {
  const cover =
...
function sanitizeNextAiringEpisode(value: string): string | null {
...
function parseRelatedAnime(html: string, baseUrl: string): AnimeDetail['related'] {
  const relatedBlock =
    html.match(
      /<h2><i class="icon fas fa-burn icon-yellow"><\/i>Relacionados<\/h2>[\s\S]*?<div class="articulos[^"]*">([\s\S]*?)<\/div>\s*<\/div>\s*<div id="animes-right-side">/i,
    )?.[1] ?? ''

  const related: AnimeDetail['related'] = []
...
function parseDownloadMap(html: string, baseUrl: string): Map<string, string> {
...
function parseEpisodeServers(html: string, baseUrl: string): EpisodeDetail['servers'] {
...
export function createAnimeMovilProvider(
  baseUrl = DEFAULT_ANIMEMOVIL_BASE_URL,
): AnimeProvider {
  ...
  async function fetchAnimeDetail(slug: string, signal?: AbortSignal): Promise<AnimeDetail> {
    ...
    const title =
      normalizeText(html.match(/<div class="titles">[\s\S]*?<h1>([\s\S]*?)<\/h1>/i)?.[1] ?? '') ||
    ...
    const type = normalizeType(
      html.match(/id="show-data"[\s\S]*?<span class="item bl">([\s\S]*?)<\/span>/i)?.[1] ?? '',
    )
    const status = normalizeText(
      html.match(/<span class="estado[^"]*">([\s\S]*?)<\/span>/i)?.[1] ??
    ...
    const genres = Array.from(genresBlock.matchAll(/<a class="item br"[^>]*>([\s\S]*?)<\/a>/gi))
      .map((match) => normalizeText(match[1] ?? ''))
      .filter(Boolean)
    const nextAiringEpisode = sanitizeNextAiringEpisode(
      html.match(/<div class="prox-episodio">[\s\S]*?<span>([\s\S]*?)<\/span>/i)?.[1] ?? '',
    )
  ...
  return {
    ...
    async search(params, signal) {
      ...
      const html = await requestAnimeMovilText(
        normalizedBaseUrl,
        buildDirectoryUrl(normalizedBaseUrl, params),
        signal,
      )
      ...
      return parseDirectoryPage(
        html,
        normalizedBaseUrl,
        normalizePaginationValue(params.page),
        query ? 'text' : 'filter',
      )
    },
    ...
    async getEpisodeByNumber(animeSlug, episodeNumber, signal) {
      const html = await requestAnimeMovilText(
        normalizedBaseUrl,
        buildEpisodeUrl(normalizedBaseUrl, animeSlug, episodeNumber),
        signal,
      )
      const detail = await fetchAnimeDetail(animeSlug, signal).catch(() => null)
      const title =
        normalizeText(html.match(/<div class="titulo-episodio">[\s\S]*?<h1>([\s\S]*?)<\/h1>/i)?.[1] ?? '') ||
